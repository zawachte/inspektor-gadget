...
- gadget-core: Containes only CO-RE gadgets

You can build and push the container gadget image by running the following commands:

```bash
$ make gadget-bcc-container # or make gadget-core-container
...
`gadget.bcc.Dockerfile` to pick your own image of traceloop.
...
As you can see in `gadget.bccDockerfile`, the gadget container image
...
`gadget.bcc.Dockerfile` so that it uses the just created image, same goes for local
